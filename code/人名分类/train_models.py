...
def train_models(is_clip=False):
    ...
    # 训练参数
    lr = 0.001  # 学习率
    n_epochs = 5000  # 训练100000次（可重复的从数据集中抽取100000姓名）
    ...
    # 统一添加优化器
    losses = [[] for _ in range(len(model_names))]
    periods = [[] for _ in range(len(model_names))]
    for idx, model in enumerate(models):
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            # 每训练plot_every次，计算一个训练平均误差，方便后面可视化误差曲线图
